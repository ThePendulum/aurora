# aurora
aurora is an addressable LED controller for (but not limited to) the Raspberry Pi. It provides an easy to use web interface backed by a powerful mathematical expression parser, allowing you to create unlimited patterns or even draw pixels by hand! aurora has default support for the WS2801, WS2811, WS2812 and WS2812B chips and allows for plugins to drive any chip or output.
...
## Installation
aurora relies on Node.js, which is best installed through [nvm](https://github.com/creationix/nvm). Unfortunately, aurora requires root privileges to access the hardware required to control the LEDs. Therefore, Node.js must be installed for both your personal account and root.

### Dependencies
Node.js comes with a package manager npm, which is used to install aurora's dependencies. Please make sure to sign out as root and use your personal account:
...
### Running aurora
Please walk through the [configuration](#configuration) before running aurora for the first time. [As root](#installation), aurora can be started through npm:
